AssetsLinkEditor.funcMaps("window.loadBody", "window.highlight", "window.renderImage");
window.__DEV__ = AssetsLinkEditor.keys.__DEV__;
AssetsLinkEditor.loadFiles("nativecompressorJs");
...
const nrContentToString = async (content) => {
    try {
        if (!content)
            return content;
        let c = new Compressor(content);
        if (c.isCompressed(content))
            return c.deCompress();
    } catch (e) {
        if (window.__DEV__)
            alert(e);
...
const trySplit = function (htmlString) {
    ...
    let items = [txt.substring(0, index), txt.substring(index)].filter(x => x && x.trim().length > 0).map(x => `<p>${x}</p>`);
    if (items.length < 2)
        return undefined;
    return items;
}
...
window.imageRetry = (event, btn) => {
    btn.parentElement.classList.remove('error', 'loaded');
    const img = btn.parentElement.querySelector("img");

    const src = `${img.getAttribute("src").replace(/\?(\s)?time(\s)?=.*/ig, "").trim()}?time=${Date.now()}`
...
const loadImages = (view, option) => {
    ...
    for (let img of imgs) {
        ...
        if (src.length > 0 && !src.startsWith("data:")) {
            img.setAttribute("src", `${document.body.getAttribute("imageaddress")}/${encodeURIComponent(src)}/${img.id}`);
...
const create = async (option) => {
    try {
        ...
        if (option.addPrev && ["Pagination", "PaginationScroll", "Player"].includes(option.scrollType)) {
            ...
            mock.innerHTML = `<p>${option.prevText}</p>`;
        ...
        if (option.addNext && ["Pagination", "PaginationScroll", "Player"].includes(option.scrollType)) {
            const mock = createView();
            mock.classList.add("emptyView");
            mock.innerHTML = `<p>${option.nextText}</p>`;
            slider.appendChild(mock);
        }
        ...
        function createSelectionMenu(menuItem) {
            ...
            function createMenu(x, y) {
                ...
                menuItem.items.forEach(item => {
                    ...
                    div.innerHTML = `<span class='material-symbols-outlined'>${item.icon}</span>${item.text ?? ""}`;
                    div.onclick = () => {
                        const mItem = {
                            ...item,
...
window.highlight = function (options) {
    ...
    let char = "";
    const reg = new RegExp(regChar, "gmi");
    while (options.length + 1 < innerHTML.length && reg.test((char = innerHTML[options.index + options.length] || ""))) {
        options.length++;
    }
...
try {
    ...
    window.onImageLoadError = (currentImage) => {
        try {
            ...
            let newSrc = `${imageAddress}/${encodeURIComponent(src)}/${img.id}`;
    ...
    window.loadBody = async (readerOption) => {
        try {
            ...
            readerOption.content = await nrContentToString(readerOption.content);
