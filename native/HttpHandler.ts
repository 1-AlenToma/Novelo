...
class HttpValue {
    ...
    get html() {
        return new Html(await this.value, this.baseurl || "");
    }
...
class HttpHandler {
    ...
    queryString(url: string, item: any) {
        for (let key in item) {
            if (url.indexOf("?") == -1) url += "?" + key + "=" + item[key];
            else url += "&" + key + "=" + item[key];
        }
        return url;
    }

    async get_html(url: string, item?: any, baseurl?: string) {
        try {
            if (item) url = this.queryString(url, item);
            const data = fetch(url, this.header());
            return new HttpValue(await data.text(), baseurl || url);
        } catch (e) {
            console.error(e);
            return new HttpValue("<div></div>");
        }
    }
