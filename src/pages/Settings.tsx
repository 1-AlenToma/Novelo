...
import { View, Text, Icon, AlertDialog, Modal, TouchableOpacity, CheckBox, FormItem, ScrollView } from "react-native-short-style";
    ...
    AlertDialog
      ...
      .then(async (answer: boolean) => {
        try {
          if (!answer) return;
          loader.show();
          let _books = await context
            .db.Books.query
            .where.column(x => x.favorit)
            .equalTo(false)
            .and.column(x => x.parserName)
            .notEqualTo("epub").toList();

          for (let b of _books)
            await context.db.deleteBook(b.id)
